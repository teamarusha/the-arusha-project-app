...
// get report info for admin table display - grabbing date/time, patient name, responder name
router.get('/', adminAuth, (req, res) => {
  ...
  let queryText = `SELECT incident.unit_notified, patient.patient_first_name, patient.patient_last_name, "user".user_first_name, "user".user_last_name, patient.id FROM incident
    ...
    JOIN "user" ON "user".id = incident.user_id ORDER BY incident.unit_notified DESC`;
...
//grabs patient care report for specific patient selected. ALL report information
router.get('/:id', (req, res) => {
  let queryText =
    ...
    LEFT JOIN cardiacarrest ON cardiacarrest.patient_cardiac_id = patient.id
    LEFT JOIN cardiac_arrest ON cardiacarrest.cardiac_arrest_id = cardiac_arrest.id
    LEFT JOIN cardiac_arrest_etiology ON cardiac_arrest_etiology.id = cardiacarrest.cardiac_arrest_etiology_id
    LEFT JOIN resuscitation_attempt ON resuscitation_attempt.id = cardiacarrest.resuscitation_attempt_id
    LEFT JOIN cardiac_arrest_witness ON cardiac_arrest_witness.id = cardiacarrest.cardiac_arrest_witness_id
    LEFT JOIN aed_use_prior ON aed_use_prior.id = cardiacarrest.aed_use_prior_id
    ...
    LEFT JOIN disposition ON disposition.patient_disposition_id = patient.id
    LEFT JOIN transport_disposition ON transport_disposition.id = disposition.transport_disposition_id
    ...
    GROUP BY patient.id, incident.id, incident_service.id, triage_cat.id, gender.id, race.id, age_units.id, 
      ...
      alcohol_drug_use.id, disposition.id, transport_disposition.id, transport_method.id, transport_mode.id, 
      destination_facility.id, injury.id, injury_cause.id, injury_location.id;`;
